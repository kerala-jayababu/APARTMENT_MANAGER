...
public sealed class UnitResidentService(AppDbContext db) : IUnitResidentService
{
        ...
        int apartmentId, int userId, CreateUnitRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var needOwner = code is "OWNED" or "RENTED" or "OWN" or "RENT" ||
                        st.StatusName.Contains("own", StringComparison.OrdinalIgnoreCase) ||
                        st.StatusName.Contains("rent", StringComparison.OrdinalIgnoreCase);
        if (needOwner && !request.PrimaryOwnerPersonId.HasValue)
            throw new InvalidOperationException("primaryOwnerPersonId is required for this status.");
