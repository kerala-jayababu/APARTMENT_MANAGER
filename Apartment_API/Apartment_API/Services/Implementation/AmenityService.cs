...
public sealed class AmenityService(AppDbContext db) : IAmenityService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var invoicer = await ResolveInvoicerAsync(apartmentId, cancellationToken).ConfigureAwait(false);
        return new Invoice
        {
            ApartmentId = apartmentId,
            Invoicer = invoicer,
    ...
    private async Task<string> ResolveInvoicerAsync(int apartmentId, CancellationToken cancellationToken)
    {
        var apt = await db.Apartments.AsNoTracking()
            .Where(x => x.IdApartment == apartmentId)
            .Select(x => new { x.AssociationName, x.ApartmentCode, x.ApartmentName })
            .FirstOrDefaultAsync(cancellationToken)
            .ConfigureAwait(false);
        if (apt is null) return "APARTMENT";

        var label = !string.IsNullOrWhiteSpace(apt.AssociationName)
            ? apt.AssociationName.Trim()
            : !string.IsNullOrWhiteSpace(apt.ApartmentCode)
                ? apt.ApartmentCode.Trim()
                : apt.ApartmentName.Trim();
        return label.Length <= 30 ? label : label[..30];
    }
